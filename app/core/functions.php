...
// Check if any of these functions are already defined in this namespace
if (!function_exists('\\app\\core\\sanitize')) {
    ...
    function sanitize($data) {
        if (is_array($data)) {
            return array_map('app\\core\\sanitize', $data);
        }
    ...
    /**
     * Récupère le chemin d'une image selon sa catégorie et son nom
     * 
     * @param string $category Catégorie de l'image (dishes, profile, restaurant)
     * @param string $name Nom de l'élément recherché
     * @param string|null $dbImagePath Chemin stocké en base de données (prioritaire)
     * @return string Chemin de l'image ou image par défaut
     */
    function getImagePath($category, $name, $dbImagePath = null) {
        // Définir une image par défaut spécifique à la catégorie ou générique
        $defaultImage = '/app/assets/images/dishes/default_dish.png'; // Image par défaut
        
        // Si on a une URL d'image en base de données, l'utiliser prioritairement
        if (!empty($dbImagePath)) {
            return $dbImagePath;
        }
        
        // Essayer de trouver l'image par son nom dans le répertoire approprié
        $nameSlug = strtolower(str_replace([' ', '.', ',', "'", '"'], '_', $name));
        
        // Liste des extensions possibles pour les images
        $extensions = ['png', 'jpg', 'jpeg', 'webp'];
        
        // Vérifier si l'une des variantes de fichiers existe
        foreach ($extensions as $ext) {
            $imagePath = "/app/assets/images/{$category}/{$nameSlug}.{$ext}";
            if (file_exists($_SERVER['DOCUMENT_ROOT'] . $imagePath)) {
                return $imagePath;
            }
        }
        
        // Vérifier dans le tableau dish_images pour la compatibilité
        global $dish_images;
        if ($category === 'dishes' && isset($dish_images[$name])) {
            return $dish_images[$name];
        }
        
        // Aucune image trouvée, retourner l'image par défaut
        return $defaultImage;
    }

    /**
     * Normalise le chemin d'une image pour s'assurer qu'elle utilise un chemin absolu correct
     */
    function normalizeImagePath($path, $type = 'restaurant') {
        $baseUrl = '/php/v1.02/Projet_Greta/app/assets';
        
        // Si le chemin est déjà absolu ou une URL externe, le retourner tel quel
        if (strpos($path, '/') === 0 || strpos($path, 'http') === 0) {
            return $path;
        }
        
        // Extraire juste le nom du fichier de n'importe quel chemin relatif
        $filename = basename($path);
        
        // Construire un chemin absolu correct
        return $baseUrl . '/images/' . $type . '/' . $filename;
    }

    /**
     * Normalise le chemin d'une photo de profil
     */
    function normalizeProfileImagePath($path) {
        return normalizeImagePath($path, 'profile');
    }
}