...
--
-- Hôte : localhost:8889
-- Généré le : mar. 11 fév. 2025 à 13:33
...
(3, 1, 'Coq au Vin', 'Traditional French dish with chicken braised in wine', 24.99, 'Main Course', 'https://example.com/coq-au-vin.jpg', 1),
(4, 2, 'Ratatouille', 'Provençal vegetable stew', 18.99, 'Vegetarian', 'https://example.com/ratatouille.jpg', 1),
(5, 1, 'Tarte Tatin', 'Upside-down caramelized apple tart', 9.99, 'Dessert', 'https://example.com/tarte-tatin.jpg', 1),
(6, 1, 'Beef Bourguignon', 'Classic French beef stew with red wine', 28.99, 'Main Course', '/images/beef-bourguignon.jpg', 1),
(7, 1, 'Salmon en Papillote', 'Steamed salmon with herbs', 26.99, 'Fish', '/images/salmon.jpg', 1),
(8, 1, 'Mushroom Risotto', 'Creamy Italian rice with mushrooms', 22.99, 'Vegetarian', '/images/risotto.jpg', 1),
(9, 1, 'Chicken Cordon Bleu', 'Breaded chicken filled with cheese and ham', 24.99, 'Main Course', '/images/cordon-bleu.jpg', 1),
(10, 2, 'Crème Brûlée', 'Classic French vanilla custard', 9.99, 'Dessert', '/images/creme-brulee.jpg', 1),
(11, 2, 'French Onion Soup', 'Traditional onion soup with cheese', 12.99, 'Starter', '/images/onion-soup.jpg', 1);
...
(19, 'marc@gmail.com', '$2y$12$4wjsGr2CgUseZTZFRs8yMOFbApmR2SfcjI8/gemJ.IKN6Ibcg7Dja', 'marc', NULL, NULL, '2025-02-11 13:15:32', '2025-02-11 13:29:50', 1, 0, NULL, NULL, 0, 0, NULL);
...
-- Ajout des rôles manquants pour les utilisateurs
INSERT INTO user_roles (user_id, role_name, assigned_at)
SELECT user_id, 'user', CURRENT_TIMESTAMP
FROM user u
WHERE NOT EXISTS (
...
-- Ajout des rôles admin manquants
INSERT INTO user_roles (user_id, role_name, assigned_at)
SELECT user_id, 'admin', CURRENT_TIMESTAMP
FROM admin_user au
WHERE NOT EXISTS (
    SELECT 1 FROM user_roles ur WHERE ur.user_id = au.user_id AND ur.role_name = 'admin'
);

-- Création de la table system_logs
...
-- Création de la table admin_actions
CREATE TABLE `admin_actions` (
  ...
  KEY `fk_admin_actions_user` (`user_id`),
  CONSTRAINT `fk_admin_actions_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Ajout d'exemples de logs système
INSERT INTO `system_logs` (`log_type`, `message`, `severity`, `ip_address`) VALUES
('LOGIN_ATTEMPT', 'Failed login attempt for user admin@example.com', 'WARNING', '192.168.1.1'),
('SYSTEM_ERROR', 'Database connection timeout', 'ERROR', '192.168.1.1'),
('USER_ACTION', 'New user registration', 'INFO', '192.168.1.2');

-- Ajout d'exemples d'actions admin
INSERT INTO `admin_actions` (`user_id`, `action_type`, `action_details`, `ip_address`) VALUES
(17, 'USER_BAN', 'Banned user ID 5 for spam', '192.168.1.1'),
(18, 'COMMENT_DELETE', 'Deleted inappropriate comment ID 15', '192.168.1.2');
...
--
ALTER TABLE `dish`
  MODIFY `dish_id` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=12;
